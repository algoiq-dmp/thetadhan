...
// .wrangler/tmp/bundle-fft0KT/strip-cf-connecting-ip-header.js
...
// src/services/dhanClient.js
var DhanClient = class {
  constructor(env2, token, clientId) {
    this.baseUrl = env2.DHAN_API_BASE || "https://api.dhan.co/v2";
    this.token = token;
  ...
  async getFunds() {
    return this._request("GET", "/fundlimit");
  }
  // ─── Orders ───
  async placeOrder(order) {
    return this._request("POST", "/orders", order);
  }
  async modifyOrder(orderId, updates) {
    return this._request("PUT", `/orders/${orderId}`, updates);
  }
  ...
  // ─── Portfolio ───
  async getPositions() {
    return this._request("GET", "/positions");
  }
  async getHoldings() {
  ...
  async getIntradayChart(body) {
    return this._request("POST", "/charts/intraday", body);
  }
  // ─── Position Actions ───
  async convertPosition(body) {
    return this._request("PUT", "/positions/convert", body);
  }
};
__name(DhanClient, "DhanClient");
...
orderRoutes.post("/place", async (c) => {
  try {
    ...
    const dhanOrder = {
      ...
      transactionType: body.side?.toUpperCase() || body.transactionType,
      // BUY / SELL
...
portfolioRoutes.post("/exit-all", async (c) => {
  try {
    ...
    const openPositions = (positions || []).filter(
      (p) => (p.netQty || p.buyQty - p.sellQty) !== 0
    );
    const results = [];
...
var instrumentRoutes = new Hono2();
...
instrumentRoutes.get("/sync", async (c) => {
  try {
    ...
    if (!res.ok)
      throw new Error(`Dhan CSV fetch failed: ${res.status}`);
    ...
    for (let i = 1; i < lines.length; i += batchSize) {
      ...
      for (let j = i; j < Math.min(i + batchSize, lines.length); j++) {
        const cols = lines[j].split(",").map((c2) => c2.trim().replace(/"/g, ""));
        if (!cols[idx.SEM_SMST_SECURITY_ID])
          continue;
        const segment = cols[idx.SEM_SEGMENT] || "";
        if (!["NSE_EQ", "NSE_FNO", "BSE_EQ"].includes(segment))
          continue;
        batch.push(
          c.env.DB.prepare(
    ...
    ).bind(JSON.stringify((/* @__PURE__ */ new Date()).toISOString())).run();
    return c.json({ success: true, message: `Synced ${count3} instruments` });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
});
...
var diagnosticsRoutes = new Hono2();
diagnosticsRoutes.get("/", async (c) => {
  ...
  return c.json({
    ...
    limits: {
      workers: {
        maxRequestsPerDay: 1e5,
        cpuMsPerRequest: 10,
        maxSubrequests: 50
      },
      d1: {
        maxStorageMB: 500,
        maxRowsReadPerDay: 5e6,
        maxRowsWrittenPerDay: 1e5,
    ...
    usage: {
      ...
      remainingCalls: Math.max(0, 1e5 - apiUsage.totalCalls),
      usagePercent: (apiUsage.totalCalls / 1e5 * 100).toFixed(2)
    },
    // ─── Database Health ───
    database: {
...
async function getDbStats(db) {
  try {
    ...
    for (const t of tablesResult.results || []) {
      ...
      const count3 = countResult?.c || 0;
      tables[t.name] = count3;
      totalRows += count3;
      if (t.name === "instruments")
        instrumentCount = count3;
    }
...
__name(getApiUsage, "getApiUsage");
...
__name(checkDhanHealth, "checkDhanHealth");
async function getInstrumentFreshness(db) {
  try {
    ...
    if (lastSync) {
      const ageHours = (Date.now() - new Date(lastSync).getTime()) / 36e5;
      if (ageHours < 24)
        ageDescription = `${ageHours.toFixed(1)} hours ago`;
      else
        ageDescription = `${(ageHours / 24).toFixed(1)} days ago`;
    }
...
__name(getInstrumentFreshness, "getInstrumentFreshness");

// src/feed/FeedRelay.js
var FeedRelay = class {
  constructor(state, env2) {
    ...
    this.subscribedInstruments = /* @__PURE__ */ new Map();
  }
  async fetch(request) {
    const url = new URL(request.url);
      ...
      switch (msg.type) {
        ...
        case "unsubscribe":
          if (msg.instruments && Array.isArray(msg.instruments)) {
            for (const inst of msg.instruments) {
              const key = `${inst.exchange}:${inst.securityId}`;
              const count3 = (this.subscribedInstruments.get(key) || 1) - 1;
              if (count3 <= 0)
                this.subscribedInstruments.delete(key);
              else
                this.subscribedInstruments.set(key, count3);
            }
  ...
  async webSocketError(ws, error3) {
    this.clients.delete(ws);
    console.error(`[FeedRelay] Client error:`, error3);
  }
  // ─── Dhan Upstream Connection ───
  async connectToDhan() {
    if (this.dhanWs)
      return;
    if (!this.dhanToken || !this.dhanClientId)
      return;
    ...
    try {
      ...
      this.dhanWs.addEventListener("close", (event) => {
        ...
        this.dhanWs = null;
        if (this.clients.size > 0) {
          setTimeout(() => this.connectToDhan(), 5e3);
        }
      });
...
async function scheduled(event, env2, ctx) {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(`Dhan CSV fetch failed: ${res.status}`);
    const csv = await res.text();
    await syncInstrumentsToD1(csv, env2.DB);
...
__name(syncInstrumentsToD1, "syncInstrumentsToD1");
var src_default = {
  fetch: app.fetch,
  scheduled
};
...
// .wrangler/tmp/bundle-fft0KT/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fft0KT/middleware-loader.entry.ts
